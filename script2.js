...
let input_taking=document.getElementById('input-taking');
let buttons=document.getElementById('buttons');
buttons.addEventListener('click',startCounter);

function startCounter()
{
    let curr=1;
    let finish_time=parseInt(input_taking.value);


    if(finish_time<0 || finish_time>99999)
    {
        window.alert('set counter within range of 0 and 99999!!');
        return;
    }
...
function resetCounter(current,next,end)
{
    for(let i=0;i<=end;i++)
    {
